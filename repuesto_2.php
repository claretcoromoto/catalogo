...
if (!isset($email)) {
    ...
    ?>
    <!--    <input  type="reset" class="btn-mini btn-danger" ><br>
    ///********************************************************
    ...
    */-->
    ...
    <!--  
     ///********************************************************
     PAGINA FUNCIONAL (Funcional o de visualización)
     FINALIDAD:       Muestra repuesto por búsqueda de código de repuesto
     FECHA:          2014
     DESARROLLADO:    claretcoromoto@hotmail.ES  victor_rosendo@hotmail.com
     MODIFICADO:          Nombre / Fecha / #Release
     ///********************************************************
    ...
    if (isset($_POST['submit'])) {
        if (isset($_POST['cod_repuesto']) && $_POST['cod_repuesto'] != '') {
            $cod = strtoupper(htmlentities(strip_tags(strtoupper(trim($_POST['cod_repuesto'])))));    //     
       ...
       WHERE rep.cod_repuesto LIKE '$cod%' ";
            $result = @pg_query($re);
            $disponibilidad = 0;
            $encabe = '';
