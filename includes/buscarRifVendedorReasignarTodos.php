...
try {
    ...
    if ($row > 0) {
        ...
        if (isset($email) && $rol == 1 ) {
            header('Location:../AdminReasignarTodosClientes.php?rif='.$rifCliente.'&rifVende='.$rifi.'');
        ...
        if ($row === 0) {
            if (isset($email) && $rol == 1 ) {
                echo "<script language='JavaScript'> alert('El vendedor no existe')
                    location.href = '../AdminReasignarTodosClientes.php?rif=$rifCliente&rifVende=$rifi';
                              exit();  
                              </script> ";
            }
        }
