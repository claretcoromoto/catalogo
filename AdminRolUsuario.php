                                ...
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = pg_fetch_array($result)) {
                                            ?>
                                            <tr> 
                                                <td id="id"><?php echo $row['id_rol_usr'] ?></td> 
                                                <td id="nb"><?php echo $row['nb_rol_usr'] ?></td> 
                                                <?php if ($row['int_activo'] === '1') { ?>
                                                    <td id="ina"><?php echo 'Activo' ?></td>
                                                    <!--<td>  <button  type="submit" name="enviar" class="btn btn-danger" >Desactivar</button></td>-->
                                                    <td align="center"><a href="includes/ActualizarRolAdmin.php?ids=<?php echo $row['id_rol_usr'] ?>&amp;ina=0">  <button type="submit"  title="Desactivar" class="btn btn-mini btn-danger"><i class="icon-off "></i></button>   
                                                    <?php       } else { ?>
                                                    <td id="ina"><?php echo 'Inactivo' ?></td>
                                                    <td align="center"><a href="includes/ActualizarRolAdmin.php?ids=<?php echo $row['id_rol_usr'] ?>&amp;ina=1">  <button type="submit"  title="Activar" class="btn btn-mini btn-success"><i class="icon-ok "></i></button> 
                                                <?php } ?>
                                            </tr> 
                                        <?php } ?>
                                    </tbody>
